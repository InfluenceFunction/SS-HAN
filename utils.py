...
def load_acm(remove_self_loop):
    ...
    data_path = "C:\\Users\Administrator\.dgl\ACM3025.pkl"
...
def load_acm_raw(remove_self_loop):
    ...
    # download(_get_dgl_url(url), path=data_path)
    data_path = "F:\PycharmProject\SS-HAN\data\\acm\\ACM.mat"
    ...
    authors = data['PvsA'][p_selected].nonzero()[1]
    subjects = data['PvsL'][p_selected].nonzero()[1]
    re_authors, author_dic = authors_map(authors)
    re_subjects, subject_dic = subjects_map(subjects)

    hg = dgl.heterograph({
        ('paper', 'pa', 'author'): (p_vs_a.nonzero()[0], re_authors),
        ('author', 'ap', 'paper'): (re_authors, p_vs_a.nonzero()[0]),
        ('paper', 'pf', 'field'): (p_vs_l.nonzero()[0], re_subjects),
        ('field', 'fp', 'paper'): (re_subjects, p_vs_l.nonzero()[0])
    })

    # features = torch.FloatTensor(p_vs_t.toarray())
    features = get_features(data, p_selected, re_authors, author_dic, re_subjects, subject_dic)
...
def authors_map(authors):
    author_dic = {}
    re_authors = []
    for author in authors:
        if author not in author_dic:
            author_dic[author] = len(author_dic)
        re_authors.append(author_dic[author])
    re_authors = np.array(re_authors)

    return re_authors, author_dic

def subjects_map(subjects):
    subject_dic = {}
    re_subjects = []
    for subject in subjects:
        if subject not in subject_dic:
            subject_dic[subject] = len(subject_dic)
        re_subjects.append(subject_dic[subject])
    re_subjects = np.array(re_subjects)

    return re_subjects, subject_dic

def get_features(mat_file, paper_idx, re_authors, author_dic, re_subjects, subject_dic):
    ...
    subject_feat = np.array(A_ps_tmp.transpose().dot(A_pt_tmp)[len(paper_idx) + len(author_dic):len(paper_idx)
                                                                                                + len(author_dic) + len(
        subject_dic), -len(term_dic):].toarray() > 0, dtype=np.int)

    return [torch.FloatTensor(paper_feat), torch.FloatTensor(author_feat), torch.FloatTensor(subject_feat)]
